...
export class VehiclesReport extends Component {
...
search = (data={},search)=>{
    ...
    if(data.vehicle_make){
        filter.vehicle_make = data.vehicle_make
    };
    ...
    this.setState({
       ...
       .then(res=>{
        this.setState({
            vehicles:this.state.vehicles.concat(res.data.data),
            isLoading:false,
    ...
    extractReport =() =>{
        ...
        let params={
         ...
         columns : JSON.stringify([
            ...
            {"caption": ("CITY"), "align": "center"},
            {"caption": ("MAKE_YEAR"), "align": "center"}]),
         ...
         filters: this.state.filter
            ...
            .then(res =>{
                ...
                fileDownload(new Blob([res.data]), 'vehicles-report.xlsx');
    ...
    render() {
        return (
            ...
            <div className={styles.reportWrapper}>
               ...
               {this.state.vehicles.length>0?<div>
               <Paper className={styles.paper}>
               <InfiniteScroll
                        dataLength={this.state.vehicles.length} 
                        next={()=>this.search(this.state.filter,false)}
                        hasMore={this.state.hasMore}
                        loader={this.state.vehicles.length>0?<div style={{textAlign:"center",padding:3}}><CircularProgress size={20} /></div>:null}
                        height={700}
                        endMessage={
                    ...
                    <TableBody>
                        ...
                        <TableRow hover key={vehicle.vehicle_number + Math.random()}>
              ...
              {this.state.vehicles.length===0 && !this.state.isLoading?
              <div style={{display:"flex",justifyContent:"center"}}>
